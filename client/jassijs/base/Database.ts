...
import {DBArray} from "jassijs/remote/DBArray";
...
export class Database {
    ...
    async uploadType(definition) {
        ...
        return jassijs.server.call("uploadType", sdef);
    }
    /**
    ...
    async _getMember(ob, member, memberclassname, ids) {
        if (ids === undefined || ids === null) {
            ...
            var sret = await jassijs.server.call("getMember", data);
        ...
        if (field.type === "array") {
            var ret = new jassijs.base.DBArray();
            var vids = ids.split(",");
            var idsToResolve = "";
            for (var x = 0; x < vids.length; x++) {
                ...
                if (test !== undefined)
                    ...
                    var T = classes.getClass(memberclassname);
                    ob = new T();
            ...
            for (var ffield in data.relations[cl]) {
                ...
                for (var id in rels) {
                    ...
                    if (ofield.type === "object") {
                        ...
                        var arr = new jassijs.base.DBArray();
                        var adata = rels[id];
    ...
    async query(type, query, resolveFields = undefined) {
        ...
        var sret = await jassijs.server.call("query", query, parameter);
    ...
    async save(ob) {
        ...
        var ret = await jassijs.server.call("save", sob, parameter);
        if (!$.isNumeric(ret))
            throw ret;
        ob.id = Number(ret);
    ...
    async remove(ob) {
        var parameter = { type: ob.__proto__._dbtype };
        return await jassijs.server.call("remove", { id: ob.id }, parameter);
    }
    ...
    */
    ...
    getTypeDef(classname) {
        ...
        if (this[sclassname] === undefined) {
            ...
            //var def=JSON.parse(ret.responseText);
            this[sclassname] = new jassijs.base.Database.Type(sclassname);
            debugger;
            alert("is the next line right?");
            var def = this[sclassname];
            for (var x = 0; x < def.fields.length; x++) {
                var jf = def.fields[x];
                var field = new jassijs.base.Database.Field(jf.name, jf.type, jf.usertype, jf.link);
                this[sclassname][field.name] = field;
                if (field.link !== undefined) {
                    this[sclassname].linkedfields[field.link] = field;
                }
