interface BrowserServerWorkerInterface {
    readIndexDB: (dbName: string, storeName: string, key: string) => Promise<any>;
    _openDB: (dbName: string, storeName: string) => Promise<void>;
    writeIndexDB: (dbName: string, storeName: string, key: string, value: any) => Promise<any>;
    keysIndexDB: (dbName: string) => Promise<string[]>;
    
    myindexdb: IDBDatabase;
}

browserserverworker.readIndexDB = async (dbName: string, storeName: string, key: string) => {
    key = storeName + "-" + key;
    storeName = "[system]";
...
browserserverworker._openDB = async (dbName: string, storeName: string) => {
    if (browserserverworker.myindexdb != undefined)
        return;
    browserserverworker.myindexdb = await new Promise<IDBDatabase>((resolve, reject) => {
        ...
        checkRequest.onupgradeneeded = (event: any) => {
...
browserserverworker.writeIndexDB = async (dbName: string, storeName: string, key: string, value: any) => {
    key = storeName + "-" + key;
    storeName = "[system]";
    await browserserverworker._openDB(dbName, storeName);
...
//@ts-ignore
browserserverworker.keysIndexDB = async (dbName: string) => {
