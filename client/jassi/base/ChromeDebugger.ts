...
import { Debugger } from "jassi/base/Debugger";
 ...
 * debugging in Chrome
 */
@$Class("jassi.base.ChromeDebugger")
export class ChromeDebugger extends Debugger {
    ...
    async saveCode(url, code) {
        ...
        var editor = windows.findComponent("jassi.ui.CodeEditor-" + filename);
        if (editor !== undefined) {
    ...
    */
    ...
    reportVariables(url, variables, type = undefined) {
        ...
        var editor = jassi.windows.findComponent("jassi.ui.CodeEditor-" + file);
        if (editor !== undefined) {
