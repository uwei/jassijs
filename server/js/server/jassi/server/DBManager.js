...
function getConOpts() {
    ...
    try {
        var list = new Filessystem_1.default().dirFiles("../client/remote", [".ts"]);
...
class DBManager {
    ...
    async save(entity, options) {
        ...
        if (Classes_1.classes.getClassName(entity) === "remote.jassi.security.User" && entity.password !== undefined) {
...
class RelationInfo {
    ...
    async addParentRightDestriction(builder) {
        ...
        var query = this.dbmanager.connection().createQueryBuilder().
            select("me").from(Classes_1.classes.getClass("remote.jassi.security.ParentRight"), "me").
