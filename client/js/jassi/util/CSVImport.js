...
`;
    let CSVImport = class CSVImport extends Panel_1.Panel {
        constructor() {
            super();
            this.me = {};
            this.layout(this.me);
        ...
        initTableHeaders() {
            ...
            var meta = Database_1.db.getMetadata(Classes_1.classes.getClass(this.me.select.value));
            var lkeys = [];
            for (var key in meta) {
                if (key === "this")
                    continue;
        ...
        async doimport() {
            //read userchoices
            var assignedfields = {};
            var Type = Classes_1.classes.getClass(this.me.select.value);
            //read objects so we can read from cache
            let nil = await Type["find"]();
            var meta = Database_1.db.getMetadata(Classes_1.classes.getClass(this.me.select.value));
            var members = Registry_1.default.getMemberData("design:type")[this.me.select.value];
            for (var x = 0; x < this.fieldCount; x++) {
            ...
            for (var x = from; x < this.data.length; x++) {
                ...
                var exists = DBObject_1.DBObject.getFromCache(this.me.select.value, ob.id);
                if (exists) {
    ...
    async function test() {
        var t = await Classes_1.classes.loadClass("northwind.Customer");
