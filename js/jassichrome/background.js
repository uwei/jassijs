...
function changeBreakpoint(request, sender, sendRes) {
    ...
    enableDebuggerIfNeeded(true, sender.tab, () => {
        if (request.enable) {
            try {
                var sicres = sendRes;
                var test = chrome.debugger.sendCommand(debuggeeId, "Debugger.setBreakpointByUrl", { condition: "", lineNumber: request.line, url: request.url }, (data1, data2) => {
                    var h = data1;
                    if (!h)
                        h = {};
                    sendRes(h);
            ...
            try {
                ...
                chrome.debugger.sendCommand(debuggeeId, "Debugger.removeBreakpoint", { breakpointId: bid }, function (data1, data2) {
                    var h = data1;
                    if (!h)
                        h = {};
                    sendRes(h);
