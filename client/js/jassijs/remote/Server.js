        ...
        async saveFiles(fileNames, contents, context = undefined) {
            if (!(context === null || context === void 0 ? void 0 : context.isServer)) {
                ...
                if (res === "") {
                    ...
                    //@ts-ignore
                    new Promise((resolve_2, reject_2) => { require(["jassijs/ui/Notify"], resolve_2, reject_2); }).then(__importStar).then((el) => {
       ...
       **/
        async testServersideFile(name, context = undefined) {
            ...
            if (!(context === null || context === void 0 ? void 0 : context.isServer)) {
                ...
                //@ts-ignore
                var test = (await new Promise((resolve_3, reject_3) => { require([name.replaceAll("$serverside/", "")], resolve_3, reject_3); }).then(__importStar)).test;
                var ret;
                if (test)
                    ret = await test();
                return ret;
