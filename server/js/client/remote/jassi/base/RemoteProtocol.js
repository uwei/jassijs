...
let RemoteProtocol = class RemoteProtocol {
    ...
    static async simulateUser(user = undefined, password = undefined) {
        ...
        if (user === undefined) {
            //@ts-ignore
            var Cookies = (await Promise.resolve().then(() => require("jassi/util/Cookies"))).Cookies;
        ...
        try {
            ...
            if (ex.status === 401 || (ex.responseText && ex.responseText.indexOf("jwt expired") !== -1)) {
                redirect = new Promise((resolve) => {
                    //@ts-ignore
                    Promise.resolve().then(() => require("jassi/base/LoginDialog")).then((lib) => {
...
RemoteProtocol = __decorate([
    Jassi_1.$Class("remote.jassi.base.RemoteProtocol")
], RemoteProtocol);
