...
interface BrowserServerWorkerInterface {
    ...
    handleAPI: (event: any) => Promise<any>;
}
var browserserverapihandler = {
    apis: {},
    addAPI(method: string, path: string, handler: any) {
        //@ts-ignore
        this.apis[method + " " + path] = handler;
    }
};
browserserverworker.handleAPI = async (request: any) => {
...
browserserverapihandler.addAPI("post", "app", async (data: BrowserServerAppConfig) => {
    //@ts-ignore
    data.hasModified = true;
    await browserserverworker.writeIndexDB("browserserver", data.name, "config", data);
    return data;
});
browserserverapihandler.addAPI("get", "app", async (data: any, query: string) => {

    var ret = await browserserverworker.readIndexDB("browserserver", query, "config");
    return ret;
});
browserserverapihandler.addAPI("get", "apps", async (data: any, query: string) => {
