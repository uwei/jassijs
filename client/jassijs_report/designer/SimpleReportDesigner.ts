    ...
    static clone(obj) {
		...
		for (var key in obj) {
			if (Object.prototype.hasOwnProperty.call(obj, key)) {
				...
				temp[key] = SimpleReportDesigner.clone(obj[key]);
   ...
   */
    static objectToJson(value, space = undefined, nameWithQuotes: boolean = true) {
        ...
        if (nameWithQuotes === false)
            ovalue = SimpleReportDesigner.replaceQuotes(SimpleReportDesigner.clone(ovalue));
    ...
    propertyChanged() {
        ...
        delete job.data;
        let ob = SimpleReportDesigner.objectToJson(job, undefined, false);
...
export function test() {
    var ace = new AcePanelSimple();
    var codeEditor = new CodeEditor({ codePanel: ace ,hideToolbar:true});
