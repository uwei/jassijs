                    ...
                    if (event.request.url.startsWith(self.location.origin) && response) {
                        ...
                        if (dat !== undefined) {
                            return fetch(s, { cache: "no-store" }).then(function (networkResponse) {
                                var w = refetchIfNeeded(s, event, response);
                                event.waitUntil(w);
                                return w.then((newresponse) => {
                                    if (newresponse.headers.get("X-Custom-UpToDate") === "true") {
...
//# sourceMappingURL=service-worker%20copy.js.map